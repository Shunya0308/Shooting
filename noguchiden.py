...
def move_starship(scrn, key): # 自機の移動
    ...
    if key[K_RIGHT] == 1:   #右
        ...
        if ss_x > 920:
            ss_x = 920
    key_spc = (key_spc+1)*key[K_SPACE]  #スペースで単発発射
    if key_spc%5 == 1:
        set_missile(0)
        se_shot.play()
    key_z = (key_z+1)*key[K_z]  #Zキーで弾幕発射
    if key_z == 1 and ss_shield > 10:
        set_missile(10)
        ss_shield = ss_shield - 10
        se_barrage.play()

    if ss_muteki%2 == 0:    #無敵時間
...
def bring_enemy(): # 敵を出す
    ...
    if 150 < sec and sec < 175: # 150～175秒 ２種類
        if tmr%20 == 0:
            set_enemy(random.randint(20, 940), LINE_B, 270, EMY_ZAKO, 8, 1) # 敵1 下から出る
    ...
    if tmr == 30*270: # ボス出現
        ...
        pygame.mixer.music.load("sound_gl/bossStart.ogg")
        pygame.mixer.music.play(1)
        set_enemy(480, -210, 90, EMY_BOSS, 4, 200)
        
    if tmr == 30*276: #ボス戦開始
        pygame.mixer.music.stop()
        pygame.mixer.music.load("sound_gl/boss_bgm.ogg")
        pygame.mixer.music.play(-1)
...
def move_enemy(scrn): # 敵機の移動
    global idx, tmr, ss_x, ss_y, score, hisco, new_record, ss_shield
    for i in range(ENEMY_MAX):
        if emy_f[i] == True:
            ang = -90-emy_a[i]
            png = emy_type[i]
            if emy_type[i] < EMY_BOSS: # ザコ敵の動き
                emy_x[i] = emy_x[i] + emy_speed[i]*math.cos(math.radians(emy_a[i]))
                emy_y[i] = emy_y[i] + emy_speed[i]*math.sin(math.radians(emy_a[i]))
                if emy_type[i] == 1 and emy_y[i] > 120 and emy_f2[i] == False:   #弾を発射
                    set_enemy(emy_x[i],emy_y[i],90,0,15,0)
                    emy_f2[i] = True
                if emy_type[i] == 2 and emy_y[i] > 120 and emy_f2[i] == False:   #弾を発射
                    set_enemy(emy_x[i],emy_y[i],90,0,20,0)
                    emy_f2[i] = True
...
def main(): # メインループ
    ...
    pygame.init()
    pygame.display.set_caption("乃颶致伝")
